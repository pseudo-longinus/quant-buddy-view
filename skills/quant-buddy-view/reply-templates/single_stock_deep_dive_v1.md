...
- 七个 `##` 章节标题全部保留，顺序不可变化；整节无字段时保留标题并写“本轮未返回可核验的该章节数据”，消息面固定写“本轮未纳入可核验消息面”。
...
- 整篇最多使用 5 张 Markdown 表格，固定用于行情估值、财务主表、资金交易、计算维度、波动风险；资产负债、财务补充、窗口统计、消息面和持仓建议使用列表或行内文本。没有数据的表格不要输出空表。
...
{其余非空维度继续追加到同一张表；没有计算维度时写“本轮未返回可核验的该章节数据”。}
