...
<p align="center">
  <strong>把投研逻辑，发布成一个会自动更新的网页链接。</strong>
</p>
...
这些截图直接来自 [QuantBuddy 模板市场](https://www.quantbuddy.cn/templates)。它们不是静态 PPT，而是可打开、可复用、会实时取数的模板页面；点击截图可查看线上样例。
...
如果你第一次使用，建议先从官方模板开始。最快的成功路径是：

1. 安装 `quant-buddy-view` skill。
2. 配置 QuantBuddy API Key。
3. 在 [模板市场](https://www.quantbuddy.cn/templates) 选一个接近你需求的模板。
4. 让 Agent 替换标的、指标、文案和公式包。
5. 发布到 `pages.quantbuddy.cn`，得到一个可分享、会自动更新的网页链接。
...
> 注册公式包、上传页面、更新页面需要 API Key；已发布看板内的实时取数不需要 API Key。不要把真实 Key 写进公开仓库。
...
| Serverless 公式任务包 | 每个看板背后是一组专属公式和读取模式，按你的问题定制 |
...
真正可复用的公共模板在 [QuantBuddy 模板市场](https://www.quantbuddy.cn/templates)，由平台接口实时下发，不是写死在本仓库里。每个模板都可以：
...
本仓库 `skills/quant-buddy-view/examples/` 放的是离线参考模板，用来演示各类看板长什么样、版式怎么搭。它们是示例与起点，不是模板市场里的官方模板。

| 示例 | 展示内容 |
|---|---|
| `single-stock` | 个股速览：数字卡（最新价 / 涨跌幅 / 20日 / 60日 / PE / PB / 成交额）+ 价格主图 |
| `valuation-financial-profile` | 个股估值体检：PE/PB/PCF 历史水位、财务趋势、现金流质量、估值变动归因 |
| `index-anomaly` | 成分股异动榜：异动榜单 + 涨跌分布 + 指数 sparkline |
| `multi-factor-screener` | 多因子选股：主题股票池 + 多因子评分 + TopN + 回测/rankIC + K 线 + 公式审计 |
| `commodity-daily` | 商品多空日报：板块多空主导 + 今日异动 + 头部品种 sparkline |
| `bubble-watch` | 泡沫监测终端：综合温度仪表盘 + 多市场泡沫水位 + 宏观背景 |

### 自定义页面

公共模板和示例只是起点。如果你需要更贴近自己业务的问题、版式或交互，可以让 Agent 定制一张专属看板，再用 quant-buddy-view 发布成同样可分享、会实时更新的网页链接。进阶做法见 `skills/quant-buddy-view/guides/bespoke-page.md`。
...
- **Node.js 18+（可选）**：仅 `scripts/verify_page.mjs` 页面验收需要。
...
| 下载模板 HTML / 复制提示词 | 不消耗 RU |
| 注册公式任务包、刷新公式包、轮换签名、更新需要重算的数据 | 按公式条数、数据量和计算复杂度计 RU |

> 当前处于限时免费体验中：创建和刷新看板暂不消耗 RU。免费期结束后，复杂计算会按 RU 消耗；费用计入公式包所有者的配额，访问者免 API Key、零配置打开。实际消耗以平台账户页和接口返回为准。
...
<p align="center">
  ...
  <br/>
  <sub>扫码添加微信或加入交流群，欢迎交流量化数据看板、AI Agent 工作流和策略验证案例。</sub>
</p>
